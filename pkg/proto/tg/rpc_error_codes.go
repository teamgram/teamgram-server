...
A full human-readable JSON list of RPC errors that can be returned by all methods in the API can be found [here »](<https://corefork.telegram.org/file/400780400111/3/RF-b0LDHWpc.202549.json/9b83afb26f1ba2f8aa>), what follows is a description of its fields:
  ...
  * `business_supported` \- The full list of methods that can be used by bots over a [business connection with invokeWithBusinessConnection](<bots_connected-business-bots.md>).
...
const (
	/*
		...
		In all these cases, the error description's string literal contains the number of the data center (instead of the X) to which the repeated query must be sent. [More information about redirects between data centers »](<datacenter.md>)
	*/
	ErrSeeOther = 303
	...
	*/
	...
	/*
		...
		Do not display any visible error to the user when receiving the `rpc_error` constructor: instead, wait for an [updateServiceNotification](../constructor/updateServiceNotification.md) update, and handle it as usual.
		Basically, an [updateServiceNotification](../constructor/updateServiceNotification.md) `popup` update will be emitted independently (ie NOT as an [Updates](../type/Updates.md) constructor inside `rpc_result` but as a normal update) immediately after emission of a 406 `rpc_error`: the update will contain the actual localized error message to show to the user with a UI popup.
		...
		Also note that by session we mean a logged-in session identified by an [authorization](../constructor/authorization.md) constructor, fetchable using [account.getAuthorizations](../methods/method/account.getAuthorizations.md), not an MTProto session.
	...
	*/
	...
	/*
		...
		#### Error Example:
		  ...
		  * FLOOD_PREMIUM_WAIT_X: A wait of X seconds is required (where X is a number); the user may also purchase a [Telegram Premium subscription](<premium.md>) to remove this limitation. See [here »](<files.md>) for more info on how to handle this error.
	*/
	ErrFlood = 420
...
var (
    ...
    Err400MethodInvalid = ecode.NewCodeError(ErrBadRequest, "METHOD_INVALID")
    ...
    Err400PeerIdInvalid = ecode.NewCodeError(ErrBadRequest, "PEER_ID_INVALID")
    ...
    ErrBotFallbackUnsupported = ecode.NewCodeError(ErrBadRequest, "BOT_FALLBACK_UNSUPPORTED")
    ...
    ErrBotInvoiceInvalid = ecode.NewCodeError(ErrBadRequest, "BOT_INVOICE_INVALID")
    ...
    ErrBusinessConnectionInvalid = ecode.NewCodeError(ErrBadRequest, "BUSINESS_CONNECTION_INVALID")

	// ErrBusinessConnectionNotAllowed
    // | 400 | BUSINESS_CONNECTION_NOT_ALLOWED | This method was invoked over a business connection using [invokeWithBusinessConnection](https://core.telegram.org/api/business#connected-bots), but either (1) we're a user, and users cannot invoke methods over a business connection; (2) we're a bot, but business mode was disabled in @botfather or (3); we're a bot, but this method cannot be invoked over a business connection. |
    ErrBusinessConnectionNotAllowed = ecode.NewCodeError(ErrBadRequest, "BUSINESS_CONNECTION_NOT_ALLOWED")
    ...
    ErrBusinessPeerUsageMissing = ecode.NewCodeError(ErrBadRequest, "BUSINESS_PEER_USAGE_MISSING")
    ...
    ErrButtonCopyTextInvalid = ecode.NewCodeError(ErrBadRequest, "BUTTON_COPY_TEXT_INVALID")
    ...
    ErrButtonIdInvalid = ecode.NewCodeError(ErrBadRequest, "BUTTON_ID_INVALID")

	// ErrButtonInvalid
    // | 400 | BUTTON_INVALID | The specified button is invalid. |
    ErrButtonInvalid = ecode.NewCodeError(ErrBadRequest, "BUTTON_INVALID")
    ...
    ErrCallProtocolLayerInvalid = ecode.NewCodeError(ErrBadRequest, "CALL_PROTOCOL_LAYER_INVALID")
    ...
    ErrChannelMonoforumUnsupported = ecode.NewCodeError(ErrBadRequest, "CHANNEL_MONOFORUM_UNSUPPORTED")
    ...
    ErrChargeIdEmpty = ecode.NewCodeError(ErrBadRequest, "CHARGE_ID_EMPTY")

	// ErrChargeIdInvalid
    // | 400 | CHARGE_ID_INVALID | The specified charge_id is invalid. |
    ErrChargeIdInvalid = ecode.NewCodeError(ErrBadRequest, "CHARGE_ID_INVALID")
    ...
    ErrChatMemberAddFailed = ecode.NewCodeError(ErrBadRequest, "CHAT_MEMBER_ADD_FAILED")
    ...
    ErrChatlistsTooMuch = ecode.NewCodeError(ErrBadRequest, "CHATLISTS_TOO_MUCH")
    ...
    ErrDataHashSizeInvalid = ecode.NewCodeError(ErrBadRequest, "DATA_HASH_SIZE_INVALID")
    ...
    ErrEffectIdInvalid = ecode.NewCodeError(ErrBadRequest, "EFFECT_ID_INVALID")
    ...
    ErrExpiresAtInvalid = ecode.NewCodeError(ErrBadRequest, "EXPIRES_AT_INVALID")
    ...
    ErrExtendedMediaAmountInvalid = ecode.NewCodeError(ErrBadRequest, "EXTENDED_MEDIA_AMOUNT_INVALID")

	// ErrExtendedMediaInvalid
    // | 400 | EXTENDED_MEDIA_INVALID | The specified paid media is invalid. |
    ErrExtendedMediaInvalid = ecode.NewCodeError(ErrBadRequest, "EXTENDED_MEDIA_INVALID")
    ...
    ErrFilePartTooSmall = ecode.NewCodeError(ErrBadRequest, "FILE_PART_TOO_SMALL")
    ...
    ErrFormIdEmpty = ecode.NewCodeError(ErrBadRequest, "FORM_ID_EMPTY")

	// ErrFormSubmitDuplicate
    // | 400 | FORM_SUBMIT_DUPLICATE | The same payment form was already submitted.  . |
    ErrFormSubmitDuplicate = ecode.NewCodeError(ErrBadRequest, "FORM_SUBMIT_DUPLICATE")

	// ErrFormUnsupported
    // | 400 | FORM_UNSUPPORTED | Please update your client. |
    ErrFormUnsupported = ecode.NewCodeError(ErrBadRequest, "FORM_UNSUPPORTED")
    ...
    ErrFrozenParticipantMissing = ecode.NewCodeError(ErrBadRequest, "FROZEN_PARTICIPANT_MISSING")
    ...
    ErrGiftMonthsInvalid = ecode.NewCodeError(ErrBadRequest, "GIFT_MONTHS_INVALID")
    ...
    ErrGiftStarsInvalid = ecode.NewCodeError(ErrBadRequest, "GIFT_STARS_INVALID")
    ...
    ErrHashSizeInvalid = ecode.NewCodeError(ErrBadRequest, "HASH_SIZE_INVALID")
    ...
    ErrIdExpired = ecode.NewCodeError(ErrBadRequest, "ID_EXPIRED")

	// ErrIdInvalid
    // | 400 | ID_INVALID | The passed ID is invalid. |
    ErrIdInvalid = ecode.NewCodeError(ErrBadRequest, "ID_INVALID")
    ...
    ErrInputPeersEmpty = ecode.NewCodeError(ErrBadRequest, "INPUT_PEERS_EMPTY")

	// ErrInputPurposeInvalid
    // | 400 | INPUT_PURPOSE_INVALID | The specified payment purpose is invalid. |
    ErrInputPurposeInvalid = ecode.NewCodeError(ErrBadRequest, "INPUT_PURPOSE_INVALID")
    ...
    ErrInviteSlugInvalid = ecode.NewCodeError(ErrBadRequest, "INVITE_SLUG_INVALID")
    ...
    ErrInvoiceInvalid = ecode.NewCodeError(ErrBadRequest, "INVOICE_INVALID")
    ...
    ErrMediaAlreadyPaid = ecode.NewCodeError(ErrBadRequest, "MEDIA_ALREADY_PAID")
    ...
    ErrMonthInvalid = ecode.NewCodeError(ErrBadRequest, "MONTH_INVALID")
    ...
    ErrMsgVoiceMissing = ecode.NewCodeError(ErrBadRequest, "MSG_VOICE_MISSING")
    ...
    ErrNoPaymentNeeded = ecode.NewCodeError(ErrBadRequest, "NO_PAYMENT_NEEDED")

	// ErrNogeneralHideForbidden
    // | 400 | NOGENERAL_HIDE_FORBIDDEN | Only the "General" topic with `id=1` can be hidden. |
    ErrNogeneralHideForbidden = ecode.NewCodeError(ErrBadRequest, "NOGENERAL_HIDE_FORBIDDEN")
    ...
    ErrPackTypeInvalid = ecode.NewCodeError(ErrBadRequest, "PACK_TYPE_INVALID")

	// ErrParentPeerInvalid
    // | 400 | PARENT_PEER_INVALID | The specified `parent_peer` is invalid. |
    ErrParentPeerInvalid = ecode.NewCodeError(ErrBadRequest, "PARENT_PEER_INVALID")
    ...
    ErrPaymentCredentialsInvalid = ecode.NewCodeError(ErrBadRequest, "PAYMENT_CREDENTIALS_INVALID")
    ...
    ErrPaymentRequired = ecode.NewCodeError(ErrBadRequest, "PAYMENT_REQUIRED")
    ...
    ErrPeerTypesInvalid = ecode.NewCodeError(ErrBadRequest, "PEER_TYPES_INVALID")
    ...
    ErrPinnedTooMuch = ecode.NewCodeError(ErrBadRequest, "PINNED_TOO_MUCH")
    ...
    ErrPricingChatInvalid = ecode.NewCodeError(ErrBadRequest, "PRICING_CHAT_INVALID")
    ...
    ErrPurposeInvalid = ecode.NewCodeError(ErrBadRequest, "PURPOSE_INVALID")
    ...
    ErrQuickRepliesBotNotAllowed = ecode.NewCodeError(ErrBadRequest, "QUICK_REPLIES_BOT_NOT_ALLOWED")
    ...
    ErrRandomIdExpired = ecode.NewCodeError(ErrBadRequest, "RANDOM_ID_EXPIRED")
    ...
    ErrReactionsCountInvalid = ecode.NewCodeError(ErrBadRequest, "REACTIONS_COUNT_INVALID")
    ...
    ErrReplyToMonoforumPeerInvalid = ecode.NewCodeError(ErrBadRequest, "REPLY_TO_MONOFORUM_PEER_INVALID")
    ...
    ErrSavedIdEmpty = ecode.NewCodeError(ErrBadRequest, "SAVED_ID_EMPTY")
    ...
    ErrSelfDeleteRestricted = ecode.NewCodeError(ErrBadRequest, "SELF_DELETE_RESTRICTED")
    ...
    ErrSendMessageGameInvalid = ecode.NewCodeError(ErrBadRequest, "SEND_MESSAGE_GAME_INVALID")
    ...
    ErrSrpAInvalid = ecode.NewCodeError(ErrBadRequest, "SRP_A_INVALID")
    ...
    ErrStargiftAlreadyConverted = ecode.NewCodeError(ErrBadRequest, "STARGIFT_ALREADY_CONVERTED")

	// ErrStargiftAlreadyRefunded
    // | 400 | STARGIFT_ALREADY_REFUNDED | The specified star gift was already refunded. |
    ErrStargiftAlreadyRefunded = ecode.NewCodeError(ErrBadRequest, "STARGIFT_ALREADY_REFUNDED")

	// ErrStargiftAlreadyUpgraded
    // | 400 | STARGIFT_ALREADY_UPGRADED | The specified gift was already upgraded to a collectible gift. |
    ErrStargiftAlreadyUpgraded = ecode.NewCodeError(ErrBadRequest, "STARGIFT_ALREADY_UPGRADED")

	// ErrStargiftInvalid
    // | 400 | STARGIFT_INVALID | The passed gift is invalid. |
    ErrStargiftInvalid = ecode.NewCodeError(ErrBadRequest, "STARGIFT_INVALID")

	// ErrStargiftNotFound
    // | 400 | STARGIFT_NOT_FOUND | The specified gift was not found. |
    ErrStargiftNotFound = ecode.NewCodeError(ErrBadRequest, "STARGIFT_NOT_FOUND")

	// ErrStargiftOwnerInvalid
    // | 400 | STARGIFT_OWNER_INVALID | You cannot transfer or sell a gift owned by another user. |
    ErrStargiftOwnerInvalid = ecode.NewCodeError(ErrBadRequest, "STARGIFT_OWNER_INVALID")

	// ErrStargiftPeerInvalid
    // | 400 | STARGIFT_PEER_INVALID | The specified inputSavedStarGiftChat.peer is invalid. |
    ErrStargiftPeerInvalid = ecode.NewCodeError(ErrBadRequest, "STARGIFT_PEER_INVALID")
    ...
    ErrStargiftResellCurrencyNotAllowed = ecode.NewCodeError(ErrBadRequest, "STARGIFT_RESELL_CURRENCY_NOT_ALLOWED")

	// ErrStargiftSlugInvalid
    // | 400 | STARGIFT_SLUG_INVALID | The specified gift slug is invalid. |
    ErrStargiftSlugInvalid = ecode.NewCodeError(ErrBadRequest, "STARGIFT_SLUG_INVALID")

	// ErrStargiftTransferTooEarly_%d
    // | 400 | STARGIFT_TRANSFER_TOO_EARLY_%d | You cannot transfer this gift yet, wait %d seconds. |
    // ErrStargiftTransferTooEarly_%d = ecode.NewCodeError(ErrBadRequest, "STARGIFT_TRANSFER_TOO_EARLY_%d")
    ...
    ErrStargiftUpgradeUnavailable = ecode.NewCodeError(ErrBadRequest, "STARGIFT_UPGRADE_UNAVAILABLE")

	// ErrStargiftUsageLimited
    // | 400 | STARGIFT_USAGE_LIMITED | The gift is sold out. |
    ErrStargiftUsageLimited = ecode.NewCodeError(ErrBadRequest, "STARGIFT_USAGE_LIMITED")
    ...
    ErrStargiftUserUsageLimited = ecode.NewCodeError(ErrBadRequest, "STARGIFT_USER_USAGE_LIMITED")
    ...
    ErrStarrefAwaitingEnd = ecode.NewCodeError(ErrBadRequest, "STARREF_AWAITING_END")

	// ErrStarrefExpired
    // | 400 | STARREF_EXPIRED | The specified referral link is invalid. |
    ErrStarrefExpired = ecode.NewCodeError(ErrBadRequest, "STARREF_EXPIRED")

	// ErrStarrefHashRevoked
    // | 400 | STARREF_HASH_REVOKED | The specified affiliate link was already revoked. |
    ErrStarrefHashRevoked = ecode.NewCodeError(ErrBadRequest, "STARREF_HASH_REVOKED")
    ...
    ErrStarrefPermilleInvalid = ecode.NewCodeError(ErrBadRequest, "STARREF_PERMILLE_INVALID")
    ...
    ErrStarrefPermilleTooLow = ecode.NewCodeError(ErrBadRequest, "STARREF_PERMILLE_TOO_LOW")

	// ErrStarsAmountInvalid
    // | 400 | STARS_AMOUNT_INVALID | The specified amount in stars is invalid. |
    ErrStarsAmountInvalid = ecode.NewCodeError(ErrBadRequest, "STARS_AMOUNT_INVALID")

	// ErrStarsInvoiceInvalid
    // | 400 | STARS_INVOICE_INVALID | The specified Telegram Star invoice is invalid. |
    ErrStarsInvoiceInvalid = ecode.NewCodeError(ErrBadRequest, "STARS_INVOICE_INVALID")
    ...
    ErrStarsPaymentRequired = ecode.NewCodeError(ErrBadRequest, "STARS_PAYMENT_REQUIRED")
    ...
    ErrSubscriptionExportMissing = ecode.NewCodeError(ErrBadRequest, "SUBSCRIPTION_EXPORT_MISSING")

	// ErrSubscriptionIdInvalid
    // | 400 | SUBSCRIPTION_ID_INVALID | The specified subscription_id is invalid. |
    ErrSubscriptionIdInvalid = ecode.NewCodeError(ErrBadRequest, "SUBSCRIPTION_ID_INVALID")

	// ErrSubscriptionPeriodInvalid
    // | 400 | SUBSCRIPTION_PERIOD_INVALID | The specified subscription_pricing.period is invalid. |
    ErrSubscriptionPeriodInvalid = ecode.NewCodeError(ErrBadRequest, "SUBSCRIPTION_PERIOD_INVALID")

	// ErrSuggestedPostAmountInvalid
    // | 400 | SUGGESTED_POST_AMOUNT_INVALID | The specified price for the suggested post is invalid. |
    ErrSuggestedPostAmountInvalid = ecode.NewCodeError(ErrBadRequest, "SUGGESTED_POST_AMOUNT_INVALID")
    ...
    ErrSuggestedPostPeerInvalid = ecode.NewCodeError(ErrBadRequest, "SUGGESTED_POST_PEER_INVALID")
    ...
    ErrTermsUrlInvalid = ecode.NewCodeError(ErrBadRequest, "TERMS_URL_INVALID")
    ...
    ErrThemeParamsInvalid = ecode.NewCodeError(ErrBadRequest, "THEME_PARAMS_INVALID")
    ...
    ErrToIdInvalid = ecode.NewCodeError(ErrBadRequest, "TO_ID_INVALID")
    ...
    ErrTodoItemDuplicate = ecode.NewCodeError(ErrBadRequest, "TODO_ITEM_DUPLICATE")

	// ErrTodoItemsEmpty
    // | 400 | TODO_ITEMS_EMPTY | A checklist was specified, but no [checklist items](https://core.telegram.org/api/todo) were passed. |
    ErrTodoItemsEmpty = ecode.NewCodeError(ErrBadRequest, "TODO_ITEMS_EMPTY")

	// ErrTodoNotModified
    // | 400 | TODO_NOT_MODIFIED | No todo items were specified, so no changes were made to the todo list. |
    ErrTodoNotModified = ecode.NewCodeError(ErrBadRequest, "TODO_NOT_MODIFIED")
    ...
    ErrTransactionIdInvalid = ecode.NewCodeError(ErrBadRequest, "TRANSACTION_ID_INVALID")
    ...
    ErrUnsupported = ecode.NewCodeError(ErrBadRequest, "UNSUPPORTED")
    ...
    ErrUserBotInvalid = ecode.NewCodeError(ErrBadRequest, "USER_BOT_INVALID")
    ...
    ErrUserGiftUnavailable = ecode.NewCodeError(ErrBadRequest, "USER_GIFT_UNAVAILABLE")
...
var (
    ...
    Err403PeerIdInvalid = ecode.NewCodeError(ErrForbidden, "PEER_ID_INVALID")
    ...
    // ErrAllowPaymentRequired_%d = ecode.NewCodeError(ErrForbidden, "ALLOW_PAYMENT_REQUIRED_%d")
    ...
    ErrBotAccessForbidden = ecode.NewCodeError(ErrForbidden, "BOT_ACCESS_FORBIDDEN")
    ...
    ErrBotVerifierForbidden = ecode.NewCodeError(ErrForbidden, "BOT_VERIFIER_FORBIDDEN")
    ...
    ErrChatSendWebpageForbidden = ecode.NewCodeError(ErrForbidden, "CHAT_SEND_WEBPAGE_FORBIDDEN")

	// ErrChatTypeInvalid
    // | 403 | CHAT_TYPE_INVALID | The specified user type is invalid. |
    ErrChatTypeInvalid = ecode.NewCodeError(ErrForbidden, "CHAT_TYPE_INVALID")
    ...
    ErrUserPermissionDenied = ecode.NewCodeError(ErrForbidden, "USER_PERMISSION_DENIED")
...
var (
    ...
    Err406PeerIdInvalid = ecode.NewCodeError(ErrNotAcceptable, "PEER_ID_INVALID")
    ...
    ErrAllowPaymentRequired = ecode.NewCodeError(ErrNotAcceptable, "ALLOW_PAYMENT_REQUIRED")

	// ErrApiGiftRestrictedUpdateApp
    // | 406 | API_GIFT_RESTRICTED_UPDATE_APP | Please update the app to access the gift API. |
    ErrApiGiftRestrictedUpdateApp = ecode.NewCodeError(ErrNotAcceptable, "API_GIFT_RESTRICTED_UPDATE_APP")
    ...
    ErrPrecheckoutFailed = ecode.NewCodeError(ErrNotAcceptable, "PRECHECKOUT_FAILED")
    ...
    ErrStargiftExportInProgress = ecode.NewCodeError(ErrNotAcceptable, "STARGIFT_EXPORT_IN_PROGRESS")
    ...
    ErrStarsFormAmountMismatch = ecode.NewCodeError(ErrNotAcceptable, "STARS_FORM_AMOUNT_MISMATCH")
    ...
    ErrTranslationsDisabled = ecode.NewCodeError(ErrNotAcceptable, "TRANSLATIONS_DISABLED")
...
var (
    ...
    ErrAuthKeyUnsynchronized = ecode.NewCodeError(ErrInternal, "AUTH_KEY_UNSYNCHRONIZED")
    ...
    // ErrAuthRestart_%d = ecode.NewCodeError(ErrInternal, "AUTH_RESTART_%d")
    ...
    ErrTranslateReqFailed = ecode.NewCodeError(ErrInternal, "TRANSLATE_REQ_FAILED")

	// ErrTranslationTimeout
    // | 500 | TRANSLATION_TIMEOUT | A timeout occurred while translating the specified text. |
    ErrTranslationTimeout = ecode.NewCodeError(ErrInternal, "TRANSLATION_TIMEOUT")
