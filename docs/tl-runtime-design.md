...
The codebase intentionally uses more than one generator.

These generators do not have identical responsibilities and should not be expected to update the same files.

### `mtprotoc`

`mtprotoc` is responsible for TL schema artifacts, including:

- `schema_*.tl.go`
- `schema_*_service.tl.go`
- `clazz_id*.tl.go`
- `clazz_name*.tl.go`
- other TL codec and registry outputs

This generator owns TL object definitions, constructor ids, predicate names, and low-level TL encode/decode shape.

### `teamgooo rpc`

`teamgooo rpc` is responsible for RPC glue and service scaffolding, including:

- generated service client/server wrappers
- RPC method argument/result glue
- app client helpers
- internal handler/service bootstrap code

This generator does not own TL schema files.

### Practical Rule

If a change is made to a TL schema template in `mtprotoc`, it should only be expected to affect output after the corresponding `mtprotoc` generation flow is run.

If a change is made to an RPC glue template in `teamgooo rpc`, it should only be expected to affect RPC wrapper and scaffolding output.

For example:

- `echo-rpc.sh` updates the RPC glue generated by `teamgooo rpc`
- it should not be expected to refresh `schema_echo.tl.go` or `schema_echo_service.tl.go`

When debugging generation mismatches, the first question should be:

- which generator owns this file?

This boundary is part of the intended design, not an incidental implementation detail.
