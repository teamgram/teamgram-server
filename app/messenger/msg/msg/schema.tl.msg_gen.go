...
func (m *TLContentMessage) Decode(d *bin.Decoder) (err error) {
    switch m.ClazzID {
        case 0x8d64b133:
             flags, err := d.Uint32()
            if err != nil { return fmt.Errorf("unable to decode contentMessage#0x8d64b133: field flags: %w", err) }
            _ = flags
			  m.Id, err = d.Int32()
if err != nil { return fmt.Errorf("unable to decode contentMessage#0x8d64b133: field id: %w", err) }
              m.DialogMessageId, err = d.Int64()
if err != nil { return fmt.Errorf("unable to decode contentMessage#0x8d64b133: field dialog_message_id: %w", err) }
             if (flags & (1 << 0)) != 0 { m.Mentioned = true }
             if (flags & (1 << 1)) != 0 { m.MediaUnread = true }
             if (flags & (1 << 2)) != 0 { m.Reaction = true }
              m.SendUserId, err = d.Int64()
if err != nil { return fmt.Errorf("unable to decode contentMessage#0x8d64b133: field send_user_id: %w", err) }
...
func (m *TLOutboxMessage) Validate(layer int32) error {
    switch clazzId := iface.GetClazzIDByName(ClazzName_outboxMessage, int(layer)); clazzId {
        case 0x539524b1:
              if err := iface.ValidateRequiredObject("message", m.Message); err != nil { return err }
        ...
        case 0x539524b1:
            ...
            var getFlags = func() uint32 {
			 ...
			 // set flags
              ...
              if m.Message == nil { return fmt.Errorf("unable to encode outboxMessage#0x539524b1: field message is nil") }
if err := m.Message.Encode(x, layer); err != nil { return fmt.Errorf("unable to encode outboxMessage#0x539524b1: field message: %w", err) }
              if m.ScheduleDate != nil { x.PutInt32(*m.ScheduleDate) }
...
func (m *TLOutboxMessage) Decode(d *bin.Decoder) (err error) {
    switch m.ClazzID {
        case 0x539524b1:
             flags, err := d.Uint32()
            if err != nil { return fmt.Errorf("unable to decode outboxMessage#0x539524b1: field flags: %w", err) }
            _ = flags
			 if (flags & (1 << 0)) != 0 { m.NoWebpage = true }
             if (flags & (1 << 1)) != 0 { m.Background = true }
              m.RandomId, err = d.Int64()
if err != nil { return fmt.Errorf("unable to decode outboxMessage#0x539524b1: field random_id: %w", err) }
              
    m.Message, err = tg.DecodeMessageClazz(d)
    if err != nil { return fmt.Errorf("unable to decode outboxMessage#0x539524b1: field message: %w", err) }

              if (flags & (1 << 2)) != 0 {
    m.ScheduleDate = new(int32)
    *m.ScheduleDate, err = d.Int32()
    if err != nil { return fmt.Errorf("unable to decode outboxMessage#0x539524b1: field schedule_date: %w", err) }
}
...
if err != nil { return fmt.Errorf("unable to decode sender#0x5a3864ba: field user_id: %w", err) }
              m.Type, err = d.Int32()
if err != nil { return fmt.Errorf("unable to decode sender#0x5a3864ba: field type: %w", err) }
              m.AuthKeyId, err = d.Int64()
if err != nil { return fmt.Errorf("unable to decode sender#0x5a3864ba: field auth_key_id: %w", err) }
