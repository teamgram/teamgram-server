...
func (c *MsgCore) MsgDeleteHistory(in *msg.TLMsgDeleteHistory) (*tg.MessagesAffectedHistory, error) {
	...
	result, err := c.processUserDialogOperation(in.UserId, in.AuthKeyId, in.PeerType, in.PeerId, deleteHistoryOperationID(in.UserId, in.PeerId, in.MaxId, in.JustClear, in.Revoke, in.AuthKeyId), body)
	if err != nil {
		return nil, err
	}
...
func deleteHistoryOperationID(userID int64, peerID int64, maxID int32, justClear bool, revoke bool, authKeyID int64) string {
	return fmt.Sprintf("v1:dialog:delete_history:user:%d:peer:%d:max:%d:clear:%t:revoke:%t:auth:%d", userID, peerID, maxID, justClear, revoke, authKeyID)
}
