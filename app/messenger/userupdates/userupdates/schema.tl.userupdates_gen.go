...
// AffectedUserOperationClazz <--
//   - TL_AffectedUserOperation
type AffectedUserOperationClazz = *TLAffectedUserOperation

func DecodeAffectedUserOperationClazz(d *bin.Decoder) (AffectedUserOperationClazz, error) {
	// id, err := d.PeekClazzID()
	id, err := d.ClazzID()
	if err != nil {
		return nil, fmt.Errorf("unable to decode AffectedUserOperation: constructor: %w", err)
	}

	switch id {
	case 0x96790279:
		x := &TLAffectedUserOperation{ClazzID: id, ClazzName2: ClazzName_affectedUserOperation}
		if err := x.Decode(d); err != nil {
			return nil, err
		}
		return x, nil
	default:
		return nil, fmt.Errorf("unable to decode AffectedUserOperation: invalid constructor %x", id)
	}

}

// TLAffectedUserOperation <--
type TLAffectedUserOperation struct {
	ClazzID         uint32             `json:"_id"`
	ClazzName2      string             `json:"_name"`
	RequesterUserId int64              `json:"requester_user_id"`
	DeliveryPolicy  int32              `json:"delivery_policy"`
	OperationKind   string             `json:"operation_kind"`
	Operation       UserOperationClazz `json:"operation"`
}

func MakeTLAffectedUserOperation(m *TLAffectedUserOperation) *TLAffectedUserOperation {
	if m == nil {
		return nil
	}
	m.ClazzName2 = ClazzName_affectedUserOperation

	return m
}

func (m *TLAffectedUserOperation) String() string {
	return iface.DebugStringWithName("affectedUserOperation", m)
}

func (m *TLAffectedUserOperation) MarshalJSON() ([]byte, error) {
	if m == nil {
		return []byte("null"), nil
	}
	return iface.MarshalWithName("affectedUserOperation", m)
}

// AffectedUserOperationClazzName <--
func (m *TLAffectedUserOperation) AffectedUserOperationClazzName() string {
	return ClazzName_affectedUserOperation
}

// ClazzName <--
func (m *TLAffectedUserOperation) ClazzName() string {
	return m.ClazzName2
}

// ToAffectedUserOperation <--
func (m *TLAffectedUserOperation) ToAffectedUserOperation() *AffectedUserOperation {
	if m == nil {
		return nil
	}
...
func (m *TLAffectedUserOperation) CalcSize(layer int32) int {
	switch clazzId := iface.GetClazzIDByName(ClazzName_affectedUserOperation, int(layer)); clazzId {
	case 0x96790279:
		size := 4
		size += 4
		size += 8
		size += 4
		size += iface.CalcStringSize(m.OperationKind)
		size += iface.CalcObjectSize(m.Operation, layer)
...
func (m *TLAffectedUserOperation) Validate(layer int32) error {
	switch clazzId := iface.GetClazzIDByName(ClazzName_affectedUserOperation, int(layer)); clazzId {
	case 0x96790279:
		if err := iface.ValidateRequiredString("operation_kind", m.OperationKind); err != nil {
			return err
		}

		if err := iface.ValidateRequiredObject("operation", m.Operation); err != nil {
			return err
		}

		return nil
	default:
		return fmt.Errorf("unable to encode affectedUserOperation: unsupported layer %d", layer)
	}
}

// Encode <--
func (m *TLAffectedUserOperation) Encode(x *bin.Encoder, layer int32) error {
	switch clazzId := iface.GetClazzIDByName(ClazzName_affectedUserOperation, int(layer)); clazzId {
	case 0x96790279:
		x.PutClazzID(0x96790279)
		...
		var flags = getFlags()
		x.PutUint32(flags)
		x.PutInt64(m.RequesterUserId)
		x.PutInt32(m.DeliveryPolicy)
		x.PutString(m.OperationKind)
		if m.Operation == nil {
			return fmt.Errorf("unable to encode affectedUserOperation#0x96790279: field operation is nil")
		}
		if err := m.Operation.Encode(x, layer); err != nil {
			return fmt.Errorf("unable to encode affectedUserOperation#0x96790279: field operation: %w", err)
		}
...
// Decode <--
func (m *TLAffectedUserOperation) Decode(d *bin.Decoder) (err error) {
	switch m.ClazzID {
	case 0x96790279:
		flags, err := d.Uint32()
		if err != nil {
			return fmt.Errorf("unable to decode affectedUserOperation#0x96790279: field flags: %w", err)
		}
		_ = flags
		m.RequesterUserId, err = d.Int64()
		if err != nil {
			return fmt.Errorf("unable to decode affectedUserOperation#0x96790279: field requester_user_id: %w", err)
		}
		m.DeliveryPolicy, err = d.Int32()
		if err != nil {
			return fmt.Errorf("unable to decode affectedUserOperation#0x96790279: field delivery_policy: %w", err)
		}
		m.OperationKind, err = d.String()
		if err != nil {
			return fmt.Errorf("unable to decode affectedUserOperation#0x96790279: field operation_kind: %w", err)
		}

		m.Operation, err = DecodeUserOperationClazz(d)
		if err != nil {
			return fmt.Errorf("unable to decode affectedUserOperation#0x96790279: field operation: %w", err)
		}

		return nil
	default:
		return fmt.Errorf("unable to decode affectedUserOperation: invalid constructor %x", m.ClazzID)
	}
}

// AffectedUserOperation <--
type AffectedUserOperation = TLAffectedUserOperation
