...
// TLUserupdatesProcessUserOperationWithEffects <--
type TLUserupdatesProcessUserOperationWithEffects struct {
	ClazzID         uint32                       `json:"_id"`
	Operation       UserOperationClazz           `json:"operation"`
	AffectedEffects []AffectedUserOperationClazz `json:"affected_effects"`
}

func (m *TLUserupdatesProcessUserOperationWithEffects) String() string {
	return iface.DebugStringWithName(ClazzName_userupdates_processUserOperationWithEffects, m)
}

// Encode <--
func (m *TLUserupdatesProcessUserOperationWithEffects) Encode(x *bin.Encoder, layer int32) error {
	switch clazzId := iface.GetClazzIDByName(ClazzName_userupdates_processUserOperationWithEffects, int(layer)); clazzId {
	case 0xbacea5bf:
		x.PutClazzID(0xbacea5bf)

		if m.Operation == nil {
			return fmt.Errorf("unable to encode userupdates_processUserOperationWithEffects#0xbacea5bf: field operation is nil")
		}
		if err := m.Operation.Encode(x, layer); err != nil {
			return fmt.Errorf("unable to encode userupdates_processUserOperationWithEffects#0xbacea5bf: field operation: %w", err)
		}

		if err := iface.EncodeObjectList(x, m.AffectedEffects, layer); err != nil {
			return fmt.Errorf("unable to encode userupdates_processUserOperationWithEffects#0xbacea5bf: field affected_effects: %w", err)
		}

		return nil
	default:
		return fmt.Errorf("unable to encode userupdates_processUserOperationWithEffects: unsupported layer %d", layer)
	}
}

// Decode <--
func (m *TLUserupdatesProcessUserOperationWithEffects) Decode(d *bin.Decoder) (err error) {
	if m.ClazzID == 0 {
		m.ClazzID, err = d.ClazzID()
		if err != nil {
			return fmt.Errorf("unable to decode userupdates_processUserOperationWithEffects: constructor: %w", err)
		}
	}
	switch m.ClazzID {
	case 0xbacea5bf:
		...
		m.Operation, err = DecodeUserOperationClazz(d)
		if err != nil {
			return fmt.Errorf("unable to decode userupdates_processUserOperationWithEffects#0xbacea5bf: field operation: %w", err)
		}

		l2, err3 := d.VectorHeader()
		if err3 != nil {
			return fmt.Errorf("unable to decode userupdates_processUserOperationWithEffects#0xbacea5bf: field affected_effects: %w", err3)
		}
		if l2 > bin.MaxVectorLen {
			return fmt.Errorf("unable to decode userupdates_processUserOperationWithEffects#0xbacea5bf: field affected_effects: %w", &bin.InvalidLengthError{Type: "vector", Length: int(l2)})
		}
		prealloc2 := int(l2)
		if prealloc2 > bin.PreallocateLimit {
			prealloc2 = bin.PreallocateLimit
		}
		v2 := make([]AffectedUserOperationClazz, 0, prealloc2)
		for i := int32(0); i < l2; i++ {
			vv2, err3 := DecodeAffectedUserOperationClazz(d)
			if err3 != nil {
				return fmt.Errorf("unable to decode userupdates_processUserOperationWithEffects#0xbacea5bf: field affected_effects: %w", err3)
			}
			v2 = append(v2, vv2)
		}
		m.AffectedEffects = v2

		return nil
	default:
		return fmt.Errorf("unable to decode userupdates_processUserOperationWithEffects: invalid constructor %x", m.ClazzID)
	}
}
...
func (m *TLUserupdatesGetOutboxReadDate) Decode(d *bin.Decoder) (err error) {
	if m.ClazzID == 0 {
		m.ClazzID, err = d.ClazzID()
		if err != nil {
			return fmt.Errorf("unable to decode userupdates_getOutboxReadDate: constructor: %w", err)
		}
	}
	switch m.ClazzID {
	case 0x56fb4ad9:
		m.UserId, err = d.Int64()
		if err != nil {
			return fmt.Errorf("unable to decode userupdates_getOutboxReadDate#0x56fb4ad9: field user_id: %w", err)
		}
		m.PeerType, err = d.Int32()
		if err != nil {
			return fmt.Errorf("unable to decode userupdates_getOutboxReadDate#0x56fb4ad9: field peer_type: %w", err)
		}
		m.PeerId, err = d.Int64()
		if err != nil {
			return fmt.Errorf("unable to decode userupdates_getOutboxReadDate#0x56fb4ad9: field peer_id: %w", err)
		}
		m.MsgId, err = d.Int32()
		if err != nil {
			return fmt.Errorf("unable to decode userupdates_getOutboxReadDate#0x56fb4ad9: field msg_id: %w", err)
		}
