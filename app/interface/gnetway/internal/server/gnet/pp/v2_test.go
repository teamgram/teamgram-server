...
func TestParseV2Header(t *testing.T) {
	tests := []struct {
		...
		{
			name: "UNSPEC family/proto with TLVs only",
			// ver_cmd: v2 + PROXY, fam: AF_UNSPEC/UNSPEC, len=7
			header: []byte{
		...
		{
			name: "Unknown family/proto combination",
			// fam = 0x41 (unknown AF with STREAM)
			header: []byte{
				0x0D, 0x0A, 0x0D, 0x0A, 0x00, 0x0D, 0x0A, 0x51, 0x55, 0x49, 0x54, 0x0A,
				0x21,       // v2 + PROXY
				0x41,       // unsupported family/proto
				0x00, 0x04, // len = 4
				0xDE, 0xAD, 0xBE, 0xEF,
			},
			err: "while parsing proxy proto v2 header: unsupported protocol family '41'",
		},
		{
			name: "Unsupported command 0x2",
			// ver_cmd low 4 bits = 0x2 (unsupported command)
			header: []byte{
				0x0D, 0x0A, 0x0D, 0x0A, 0x00, 0x0D, 0x0A, 0x51, 0x55, 0x49, 0x54, 0x0A,
				0x22, // v2 + command 0x2
				0x11, 0x00, 0x0C,
			...
			err: "while parsing proxy proto v2 header: unsupported proxy protocol v2 command '2'",
		},
	}
