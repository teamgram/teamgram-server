...
type DialogRepositoryQueriesModel interface {
	...
	SelectAuthSeqOutboxClaimCandidates(ctx context.Context, pendingStatus int32, failedRetryableStatus int32, readyBefore time.Time, publishingStatus int32, leaseExpiredBefore time.Time, limit int32) ([]DialogAuthSeqOutboxRow, error)
	SelectPublicUpdateOutboxClaimCandidates(ctx context.Context, pendingStatus int32, failedRetryableStatus int32, readyBefore time.Time, publishingStatus int32, leaseExpiredBefore time.Time, limit int32) ([]DialogPublicUpdateOutboxRow, error)
}

type DialogRepositoryQueriesTxModel interface {
	SelectSavedDialogNextPinOrder(userId int64) (*SavedDialogNextPinOrderRow, error)
	SelectAuthSeqOutboxClaimCandidates(pendingStatus int32, failedRetryableStatus int32, readyBefore time.Time, publishingStatus int32, leaseExpiredBefore time.Time, limit int32) ([]DialogAuthSeqOutboxRow, error)
	SelectPublicUpdateOutboxClaimCandidates(pendingStatus int32, failedRetryableStatus int32, readyBefore time.Time, publishingStatus int32, leaseExpiredBefore time.Time, limit int32) ([]DialogPublicUpdateOutboxRow, error)
}
...
func (m *defaultDialogRepositoryQueriesModel) SelectAuthSeqOutboxClaimCandidates(ctx context.Context, pendingStatus int32, failedRetryableStatus int32, readyBefore time.Time, publishingStatus int32, leaseExpiredBefore time.Time, limit int32) ([]DialogAuthSeqOutboxRow, error) {
	var rList []DialogAuthSeqOutboxRow
	query := "select outbox_id, user_id, source_perm_auth_key_id, target_auth_policy, operation_id, event_type, peer_type, peer_id, payload_schema_version, payload, payload_hash, `status`, attempt_count, next_retry_at, lease_owner, lease_until, published_seq, published_date, last_error_kind, last_error_message from dialog_auth_seq_outbox where ((`status` in (?, ?) and next_retry_at <= ?) or (`status` = ? and lease_until <= ?)) order by next_retry_at asc, outbox_id asc limit ? for update"
...
func (m *defaultDialogRepositoryQueriesTxModel) SelectAuthSeqOutboxClaimCandidates(pendingStatus int32, failedRetryableStatus int32, readyBefore time.Time, publishingStatus int32, leaseExpiredBefore time.Time, limit int32) ([]DialogAuthSeqOutboxRow, error) {
	var rList []DialogAuthSeqOutboxRow
	query := "select outbox_id, user_id, source_perm_auth_key_id, target_auth_policy, operation_id, event_type, peer_type, peer_id, payload_schema_version, payload, payload_hash, `status`, attempt_count, next_retry_at, lease_owner, lease_until, published_seq, published_date, last_error_kind, last_error_message from dialog_auth_seq_outbox where ((`status` in (?, ?) and next_retry_at <= ?) or (`status` = ? and lease_until <= ?)) order by next_retry_at asc, outbox_id asc limit ? for update"
...
func (m *defaultDialogRepositoryQueriesTxModel) SelectPublicUpdateOutboxClaimCandidates(pendingStatus int32, failedRetryableStatus int32, readyBefore time.Time, publishingStatus int32, leaseExpiredBefore time.Time, limit int32) ([]DialogPublicUpdateOutboxRow, error) {
	var rList []DialogPublicUpdateOutboxRow
	query := "select outbox_id, source_user_id, source_perm_auth_key_id, target_user_id, target_auth_policy, operation_id, delivery_path, public_update_type, peer_type, peer_id, payload_schema_version, payload, payload_hash, `status`, attempt_count, next_retry_at, lease_owner, lease_until, published_pts, published_pts_count, published_seq, published_date, last_error_kind, last_error_message from dialog_public_update_outbox where ((`status` in (?, ?) and next_retry_at <= ?) or (`status` = ? and lease_until <= ?)) order by next_retry_at asc, outbox_id asc limit ? for update"
