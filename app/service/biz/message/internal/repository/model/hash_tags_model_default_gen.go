...
var (
	hash_tagsFieldNames          = builder.RawFieldNames(&HashTags{})
	hash_tagsRows                = strings.Join(hash_tagsFieldNames, ",")
	hash_tagsRowsExpectAutoSet   = strings.Join(stringx.Remove(hash_tagsFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), ",")
	hash_tagsRowsWithPlaceHolder = strings.Join(stringx.Remove(hash_tagsFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), "=?,") + "=?"
...
func (m *defaultHashTagsModel) Insert2(ctx context.Context, data *HashTags) (sql.Result, error) {
	query := fmt.Sprintf("insert into `hash_tags` (%s) values (?, ?, ?, ?, ?, ?)", hash_tagsRowsExpectAutoSet)
...
func (m *defaultHashTagsModel) FindOne(ctx context.Context, id int64) (*HashTags, error) {
	query := fmt.Sprintf("select %s from hash_tags where id = ? limit 1", hash_tagsRows)
	var resp HashTags
...
func (m *defaultHashTagsModel) FindListByIdList(ctx context.Context, id ...int64) ([]HashTags, error) {
	...
	query := fmt.Sprintf("select %s from hash_tags where id in (%s)", hash_tagsRows, sqlx.InInt64List(id))
...
func (m *defaultHashTagsModel) Update2(ctx context.Context, data *HashTags) error {
	query := fmt.Sprintf("update `hash_tags` set %s where `id` = ?", hash_tagsRowsWithPlaceHolder)
...
func (m *defaultHashTagsModel) FindOneByUserIdHashTagHashTagMessageId(ctx context.Context, userId int64, hashTag string, hashTagMessageId int32) (*HashTags, error) {
	query := fmt.Sprintf("select %s from hash_tags where user_id = ? AND hash_tag = ? AND hash_tag_message_id = ? limit 1", hash_tagsRows)
	var resp HashTags
