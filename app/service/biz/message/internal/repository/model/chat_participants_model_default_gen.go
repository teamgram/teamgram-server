...
var (
	chat_participantsFieldNames          = builder.RawFieldNames(&ChatParticipants{})
	chat_participantsRows                = strings.Join(chat_participantsFieldNames, ",")
	chat_participantsRowsExpectAutoSet   = strings.Join(stringx.Remove(chat_participantsFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), ",")
	chat_participantsRowsWithPlaceHolder = strings.Join(stringx.Remove(chat_participantsFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), "=?,") + "=?"
...
type (
	chat_participantsModel interface {
...
func (m *defaultChatParticipantsModel) Insert2(ctx context.Context, data *ChatParticipants) (sql.Result, error) {
	query := fmt.Sprintf("insert into `chat_participants` (%s) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", chat_participantsRowsExpectAutoSet)
...
func (m *defaultChatParticipantsModel) FindOne(ctx context.Context, id int64) (*ChatParticipants, error) {
	query := fmt.Sprintf("select %s from chat_participants where id = ? limit 1", chat_participantsRows)
...
func (m *defaultChatParticipantsModel) FindListByIdList(ctx context.Context, id ...int64) ([]ChatParticipants, error) {
	...
	query := fmt.Sprintf("select %s from chat_participants where id in (%s)", chat_participantsRows, sqlx.InInt64List(id))
...
func (m *defaultChatParticipantsModel) Update2(ctx context.Context, data *ChatParticipants) error {
	query := fmt.Sprintf("update `chat_participants` set %s where `id` = ?", chat_participantsRowsWithPlaceHolder)
...
func (m *defaultChatParticipantsModel) FindOneByChatIdUserId(ctx context.Context, chatId int64, userId int64) (*ChatParticipants, error) {
	query := fmt.Sprintf("select %s from chat_participants where chat_id = ? AND user_id = ? limit 1", chat_participantsRows)
	var resp ChatParticipants
