...
func (s *Service) MessageGetOutboxReadDate(ctx context.Context, request *message.TLMessageGetOutboxReadDate) (*message.Vector_ReadParticipantDate, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("message.getOutboxReadDate - metadata: {%s}, request: {%s}", c.MD, request)

	r, err := c.MessageGetOutboxReadDate(request)
	if err != nil {
		return nil, err
	}

	c.Logger.Debugf("message.getOutboxReadDate - reply: {%s}", r)
	return r, err
}
