		...
		func(item interface{}) {
			...
			if err == nil && do.CloseFriend {
				closeFriendList[idx.idx] = tg.MakeContactData(&tg.TLContactData{
					...
					LastName:      tg.MakeFlagsString(do.ContactLastName),
					MutualContact: do.Mutual,
					Phone:         tg.MakeFlagsString(do.ContactPhone),
		...
		func(ctx context.Context, conn *sqlx.DB, keys ...string) (map[string]interface{}, error) {
			...
			for _, key := range keys {
				...
				contactList = append(contactList, tg.MakeContactData(&tg.TLContactData{
					...
					LastName:      tg.MakeFlagsString(contact.ContactLastName),
					MutualContact: contact.Mutual,
					Phone:         tg.MakeFlagsString(contact.ContactPhone),
