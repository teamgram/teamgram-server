...
func (s *Service) ChatEditChatParticipantRank(ctx context.Context, request *chat.TLChatEditChatParticipantRank) (*mtproto.MutableChat, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("chat.editChatParticipantRank - metadata: {%s}, request: {%s}", c.MD, request)

	r, err := c.ChatEditChatParticipantRank(request)
	if err != nil {
		return nil, err
	}

	c.Logger.Debugf("chat.editChatParticipantRank - reply: {%s}", r)
	return r, err
}
