...
import (
	...
	"github.com/teamgram/teamgram-server/v2/app/service/biz/code/internal/core"
...
func (s *Service) CodeCreatePhoneCode(ctx context.Context, request *code.TLCodeCreatePhoneCode) (*code.PhoneCodeTransaction, error) {
	...
	if err != nil {
		c.Logger.Errorf("code.createPhoneCode - error: request: %s, err: %v", request, err)
		return nil, err
	}
...
func (s *Service) CodeGetPhoneCode(ctx context.Context, request *code.TLCodeGetPhoneCode) (*code.PhoneCodeTransaction, error) {
	...
	if err != nil {
		c.Logger.Errorf("code.getPhoneCode - error: request: %s, err: %v", request, err)
		return nil, err
	}
...
func (s *Service) CodeDeletePhoneCode(ctx context.Context, request *code.TLCodeDeletePhoneCode) (*tg.Bool, error) {
	...
	if err != nil {
		c.Logger.Errorf("code.deletePhoneCode - error: request: %s, err: %v", request, err)
		return nil, err
	}
...
func (s *Service) CodeUpdatePhoneCodeData(ctx context.Context, request *code.TLCodeUpdatePhoneCodeData) (*tg.Bool, error) {
	...
	if err != nil {
		c.Logger.Errorf("code.updatePhoneCodeData - error: request: %s, err: %v", request, err)
		return nil, err
	}
