...
// SelectByGtQts
// select user_id, qts, update_type, update_data, date2 from user_qts_updates where user_id = :user_id and qts > :qts order by qts asc limit :limit
func (dao *UserQtsUpdatesDAO) SelectByGtQts(ctx context.Context, user_id int64, qts int32, limit int32) (rList []dataobject.UserQtsUpdatesDO, err error) {
	var (
		query = "select user_id, qts, update_type, update_data, date2 from user_qts_updates where user_id = ? and qts > ? order by qts asc limit ?"
		rows  *sqlx.Rows
	)
	rows, err = dao.db.Query(ctx, query, user_id, qts, limit)
...
// SelectByGtQtsWithCB
// select user_id, qts, update_type, update_data, date2 from user_qts_updates where user_id = :user_id and qts > :qts order by qts asc limit :limit
func (dao *UserQtsUpdatesDAO) SelectByGtQtsWithCB(ctx context.Context, user_id int64, qts int32, limit int32, cb func(i int, v *dataobject.UserQtsUpdatesDO)) (rList []dataobject.UserQtsUpdatesDO, err error) {
	var (
		query = "select user_id, qts, update_type, update_data, date2 from user_qts_updates where user_id = ? and qts > ? order by qts asc limit ?"
		rows  *sqlx.Rows
	)
