...
type AuthSeqUpdatesTxModel interface {
	...
	SelectByGtDate(authId int64, userId int64, date2 int64, limit int32) ([]AuthSeqUpdates, error)
}
...
// SelectByGtDate
// select auth_id, user_id, seq, update_type, update_data, date2 from auth_seq_updates where auth_id = :auth_id and user_id = :user_id and date2 > :date2 order by seq asc limit :limit
func (m *defaultAuthSeqUpdatesTxModel) SelectByGtDate(authId int64, userId int64, date2 int64, limit int32) (rList []AuthSeqUpdates, err error) {
	var (
		query  = "select auth_id, user_id, seq, update_type, update_data, date2 from auth_seq_updates where auth_id = ? and user_id = ? and date2 > ? order by seq asc limit ?"
		values []AuthSeqUpdates
	)
	err = m.tx.QueryRowsPartial(&values, query, authId, userId, date2, limit)
