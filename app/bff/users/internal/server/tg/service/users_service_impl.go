...
func (s *Service) UsersGetUsers(ctx context.Context, request *tg.TLUsersGetUsers) (*tg.VectorUser, error) {
	...
	c.Logger.Debugf("users.getUsers - metadata: {}, request: {%v}", request)
	...
	c.Logger.Debugf("users.getUsers - reply: {%v}", r)
	return r, err
}
...
func (s *Service) UsersGetFullUser(ctx context.Context, request *tg.TLUsersGetFullUser) (*tg.UsersUserFull, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("users.getFullUser - metadata: {}, request: {%v}", request)
	...
	c.Logger.Debugf("users.getFullUser - reply: {%v}", r)
	return r, err
}
...
func (s *Service) ContactsResolvePhone(ctx context.Context, request *tg.TLContactsResolvePhone) (*tg.ContactsResolvedPeer, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("contacts.resolvePhone - metadata: {}, request: {%v}", request)
	...
	c.Logger.Debugf("contacts.resolvePhone - reply: {%v}", r)
	return r, err
}
...
func (s *Service) UsersGetMe(ctx context.Context, request *tg.TLUsersGetMe) (*tg.User, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("users.getMe - metadata: {}, request: {%v}", request)
	...
	c.Logger.Debugf("users.getMe - reply: {%v}", r)
	return r, err
}
