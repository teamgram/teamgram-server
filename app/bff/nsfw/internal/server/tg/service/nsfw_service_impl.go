...
func (s *Service) AccountSetContentSettings(ctx context.Context, request *tg.TLAccountSetContentSettings) (*tg.Bool, error) {
	...
	c.Logger.Debugf("account.setContentSettings - metadata: {}, request: {%v}", request)
	...
	c.Logger.Debugf("account.setContentSettings - reply: {%v}", r)
	return r, err
}
...
func (s *Service) AccountGetContentSettings(ctx context.Context, request *tg.TLAccountGetContentSettings) (*tg.AccountContentSettings, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("account.getContentSettings - metadata: {}, request: {%v}", request)
	...
	c.Logger.Debugf("account.getContentSettings - reply: {%v}", r)
	return r, err
}
