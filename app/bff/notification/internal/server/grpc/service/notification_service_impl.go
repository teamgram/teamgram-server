...
func (s *Service) AccountRegisterDevice(ctx context.Context, request *mtproto.TLAccountRegisterDevice) (*mtproto.Bool, error) {
	...
	c.Logger.Debugf("account.registerDevice - metadata: %s, request: %s", c.MD.DebugString(), request.DebugString())
	...
	c.Logger.Debugf("account.registerDevice - reply: %s", r.DebugString())
	return r, err
}
...
func (s *Service) AccountUnregisterDevice(ctx context.Context, request *mtproto.TLAccountUnregisterDevice) (*mtproto.Bool, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("account.unregisterDevice - metadata: %s, request: %s", c.MD.DebugString(), request.DebugString())
	...
	c.Logger.Debugf("account.unregisterDevice - reply: %s", r.DebugString())
	return r, err
}
...
func (s *Service) AccountUpdateNotifySettings(ctx context.Context, request *mtproto.TLAccountUpdateNotifySettings) (*mtproto.Bool, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("account.updateNotifySettings - metadata: %s, request: %s", c.MD.DebugString(), request.DebugString())
	...
	c.Logger.Debugf("account.updateNotifySettings - reply: %s", r.DebugString())
	return r, err
}
...
func (s *Service) AccountGetNotifySettings(ctx context.Context, request *mtproto.TLAccountGetNotifySettings) (*mtproto.PeerNotifySettings, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("account.getNotifySettings - metadata: %s, request: %s", c.MD.DebugString(), request.DebugString())
	...
	c.Logger.Debugf("account.getNotifySettings - reply: %s", r.DebugString())
	return r, err
}
...
func (s *Service) AccountResetNotifySettings(ctx context.Context, request *mtproto.TLAccountResetNotifySettings) (*mtproto.Bool, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("account.resetNotifySettings - metadata: %s, request: %s", c.MD.DebugString(), request.DebugString())
	...
	c.Logger.Debugf("account.resetNotifySettings - reply: %s", r.DebugString())
	return r, err
}
...
func (s *Service) AccountUpdateDeviceLocked(ctx context.Context, request *mtproto.TLAccountUpdateDeviceLocked) (*mtproto.Bool, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("account.updateDeviceLocked - metadata: %s, request: %s", c.MD.DebugString(), request.DebugString())
	...
	c.Logger.Debugf("account.updateDeviceLocked - reply: %s", r.DebugString())
	return r, err
}
...
func (s *Service) AccountGetNotifyExceptions(ctx context.Context, request *mtproto.TLAccountGetNotifyExceptions) (*mtproto.Updates, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("account.getNotifyExceptions - metadata: %s, request: %s", c.MD.DebugString(), request.DebugString())
	...
	c.Logger.Debugf("account.getNotifyExceptions - reply: %s", r.DebugString())
	return r, err
}
