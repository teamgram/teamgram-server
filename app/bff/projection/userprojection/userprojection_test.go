...
func TestProjectUsersWrapsInvalidRequest(t *testing.T) {
	client := &fakeUserClient{err: userpb.ErrUserInvalidArgument}

	_, err := ProjectUsers(context.Background(), client, 1001, []int64{1002}, MissingExplicitInput)
	if !errors.Is(err, bizuserproj.ErrInvalidRequest) {
		t.Fatalf("error = %v, want wrapped invalid request", err)
	}
}
