...
func (s *Service) AccountCheckUsername(ctx context.Context, request *tg.TLAccountCheckUsername) (*tg.Bool, error) {
	...
	c.Logger.Debugf("account.checkUsername - metadata: {}, request: {%v}", request)
	...
	c.Logger.Debugf("account.checkUsername - reply: {%v}", r)
	return r, err
}
...
func (s *Service) AccountUpdateUsername(ctx context.Context, request *tg.TLAccountUpdateUsername) (*tg.User, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("account.updateUsername - metadata: {}, request: {%v}", request)
	...
	c.Logger.Debugf("account.updateUsername - reply: {%v}", r)
	return r, err
}
...
func (s *Service) ContactsResolveUsername(ctx context.Context, request *tg.TLContactsResolveUsername) (*tg.ContactsResolvedPeer, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("contacts.resolveUsername - metadata: {}, request: {%v}", request)
	...
	c.Logger.Debugf("contacts.resolveUsername - reply: {%v}", r)
	return r, err
}
...
func (s *Service) ChannelsCheckUsername(ctx context.Context, request *tg.TLChannelsCheckUsername) (*tg.Bool, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("channels.checkUsername - metadata: {}, request: {%v}", request)
	...
	c.Logger.Debugf("channels.checkUsername - reply: {%v}", r)
	return r, err
}
...
func (s *Service) ChannelsUpdateUsername(ctx context.Context, request *tg.TLChannelsUpdateUsername) (*tg.Bool, error) {
	c := core.New(ctx, s.svcCtx)
	c.Logger.Debugf("channels.updateUsername - metadata: {}, request: {%v}", request)
	...
	c.Logger.Debugf("channels.updateUsername - reply: {%v}", r)
	return r, err
}
